...
#categories : citadines, familiale/simple, grande famille, sport
...
customer <- customer[, -which(names(customer) == "casTaux")]
...
marketing <- marketing[, -which(names(marketing) == "categorie_predicted")]
marketing <- marketing[, -which(names(marketing) == "casTaux")]
marketing <- marketing[, -which(names(marketing) == "x")]
names(training_data)
names(testing_data)
names(marketing)
...
nb_class <- predict(nb, marketing, type = "class")
table(nb_class)


# Test du classifieur : probabilites pour chaque prediction testing_data and Marketing
...
# for marketing 
marketing$categorie_predicted <- nb_class
# Matrice de confusion
table( testing_data$categorie, nb_class)

# Matrice de confusion Marketing
table( marketing$categorie, nb_class)
...
conf_matrix <- confusionMatrix(marketing$categorie, nb_class)
...
svm <- svm(categories~., customer_EA_taux, probability=TRUE)
svm